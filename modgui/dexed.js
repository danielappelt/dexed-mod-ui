...
function() {
    // Apply scaling per form control type (only necessary for Distrho's Dexed)
    // cf. Extensibility of .attr() and .val() (https://blog.jquery.com/2011/05/03/jquery-16-released/)
    (jQuery || $).valHooks.number = {
	get: (el) => $(el).parents('[class*="asb2m10_dexed"]').length ? el.value/99 : el.value,
	set: (el, value) => el.value = $(el).parents('[class*="asb2m10_dexed"]').length ? Math.round(value*99) : value
    };
